...
def coco_full_nobg(filepath, annotations, split_name):
    ...
    for filename in tqdm(files):
        ...
        black_image = np.zerps(image.shape, np.uint8)
...
def coco_maskrect(filepath, annotations, split_name):
        ...
        for filename in tqdm(files):
            ...
            if len(object_ids) > 0:
                mask = np.zeros(image.shape, dtype=np.uint8)
                cv2.fillPoly(mask, get_mask_polygons(object_anns), ignore_mask_color)
                mask_inverse = np.ones(mask.shape).astype(np.uint8)*255 - mask
                objs = cv2.bitwise_or(image, mask_inverse)
                gray = cv2.cvtColor(objs, cv2.COLOR_BGR2GRAY)
            ...
            if len(object_ids) > 0:
                final_image = cv2.bitwise_or(objs, final_image) 
            img_name = filename.split('/')[-1]
            new_name = filepath.format(split_name, img_name)
            cv2.imwrite(new_name, final_image)
